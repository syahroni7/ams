    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['submit'])){
            ...
            if($_REQUEST['no_agenda'] == "" || $_REQUEST['no_surat'] == "" || $_REQUEST['asal_surat'] == "" || $_REQUEST['isi'] == ""
                || $_REQUEST['kode'] == "" || $_REQUEST['indeks'] == "" || $_REQUEST['tgl_surat'] == ""  || $_REQUEST['keterangan'] == "" || $_REQUEST['penerima'] == ""){
                                        ...
                                        if(!preg_match("/^[0-9.-]*$/", $tgl_surat)){
                                            ...
                                            if(!preg_match("/^[a-zA-Z0-9.,()\/ -]*$/", $keterangan)){
												...
												if(!preg_match("/^[a-zA-Z0-9.,()\/ -]*$/", $penerima)){
													$_SESSION['epenerima'] = 'Form Keterangan hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), dan kurung()';
													echo '<script language="javascript">window.history.back();</script>';
												} else {
                                            ...
                                            if($file != ""){
                                                ...
                                                $query = mysqli_query($config, "UPDATE tbl_surat_masuk SET no_agenda='$no_agenda',no_surat='$no_surat',asal_surat='$asal_surat',isi='$isi',kode='$nkode',indeks='$indeks',tgl_surat='$tgl_surat',keterangan='$keterangan',id_user='$id_user',penerima='$penerima' WHERE id_surat='$id_surat'");
        ...
        $query = mysqli_query($config, "SELECT id_surat, no_agenda, no_surat, asal_surat, isi, kode, indeks, tgl_surat, file, keterangan, id_user,penerima FROM tbl_surat_masuk WHERE id_surat='$id_surat'");
        list($id_surat, $no_agenda, $no_surat, $asal_surat, $isi, $kode, $indeks, $tgl_surat, $file, $keterangan, $id_user,$penerima) = mysqli_fetch_array($query);
            ...
            <div class="row jarak-form">
                ...
                <form class="col s12" method="POST" action="?page=tsm&act=edit" enctype="multipart/form-data">
                    ...
                    <div class="row">
						...
						<div class="input-field col s6">
                            <i class="material-icons prefix md-prefix">featured_play_list</i>
                            <input id="penerima" type="text" class="validate" name="penerima" value="<?php echo $penerima ;?>" required>
                                <?php
                                    if(isset($_SESSION['epenerima'])){
                                        $eketerangan = $_SESSION['epenerima'];
                                        echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$epenerima.'</div>';
                                        unset($_SESSION['epenerima']);
                                    }
                                ?>
